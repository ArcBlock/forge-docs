...
If you install forge with `forge-cli`, a default configuration is generated for you in `~/.forge_cli/forge_release.toml`. If you didn't know what toml is, please refer [toml-lang/toml](https://github.com/toml-lang/toml).
...
It only contains part of full configuration, upon forge start, it will merge with forge default configuration to generate a full configuration. If you'd like to know all the parameters in a forge configuration, you can grab a `forge_default.toml`, either from our github repo, or from a release (e.g. `cat ~/.forge_cli/release/forge/0.18.2/lib/forge_sdk-0.17.0/priv/forge_default.toml`):
