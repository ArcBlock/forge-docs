...
- 资产内容是以序列化后的格式保存成 [Google.Protobuf.Any](https://developers.google.com/protocol-buffers/docs/proto3#any) 存在 `data` 里。如果对方没有这个资产内容的 `protobuf` 文件，将无法直接解码资产的内容，技术上却是可以查看里面的部分字段内容。
