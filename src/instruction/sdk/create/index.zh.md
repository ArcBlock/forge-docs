...
- 协议层：定义了各种数据结构（Forge 中使用 [Protocol Buffers](https://developers.google.com/protocol-buffers) 定义）、数据变化方式（哈希、签名、加解密等）、 DID 生成过程
- 类库层：定义如何组装 Forge 能识别的消息，怎么解码从 Forge 拿回来的数据，比如对钱包的封装、对 Forge 能收发的交易消息的封装
- 客户端：定义如何连接 Forge 链节点（Forge 支持 [gRPC](https://grpc.io/) 和 [GraphQL](https://graphql.org/) 两种方式），如何执行常见的链上操作等

不同语言应用的领域会有很大不同，对应的 SDK 实现也有侧重点，比如：

- 如果是偏后端的语言 SDK，侧重实现 gRPC 就可以了
- 如果不需要支持 [ABT Wallet](https://abtwallet.io) 交互的逻辑处理，不需要实现 [DID 认证相关逻辑](https://github.com/ArcBlock/abt-did-spec#abt-did-authentication-protocol)
- 如果不需要支持丰富的数据哈希、加解密、签名算法，支持 Forge 选取的标准算法即可，标准算法指的是：哈希算法使用 [SHA3](https://en.wikipedia.org/wiki/SHA-3)、非对称加密算法使用 [ED25519](https://ed25519.cr.yp.to/)、地址编码使用 [base58btc](https://github.com/multiformats/multibase)
...
SDK 和 Forge 通信时需要使用 [Protocol Buffers](https://developers.google.com/protocol-buffers) 进行数据的编解码，这样 SDK 里面就需要知道 Forge 里面的数据结构，SDK 里面需要用到的数据结构主要有下面两部分：
...
因为往区块链写数据，通常等价于发送交易，发送交易前需要对交易做签名，再把签名放到 `Transaction` 实例上，使用 [Protocol Buffers](https://developers.google.com/protocol-buffers) 序列化之后发送出去，大致过程如下：
